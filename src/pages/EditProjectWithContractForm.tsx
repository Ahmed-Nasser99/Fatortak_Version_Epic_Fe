...
import { useProject, useUpdateProjectWithContract } from "../hooks/useProjects";
...
const EditProjectWithContractForm = () => {
  const navigate = useNavigate();
  const { id } = useParams<{ id: string }>();
  
  const { data: projectResponse, isLoading: isProjectLoading } = useProject(id!);
  const updateProjectMutation = useUpdateProjectWithContract();
    ...
    if (projectResponse?.success && projectResponse.data) {
      ...
      if (p.notes) {
          ...
          if (notesArr.length > 0) {
              setConditions(notesArr);
          }
      }

      if (p.projectLines && p.projectLines.length > 0) {
          // Group by section name
          const groupedMap = new Map<string, ProjectLineForm[]>();
          p.projectLines.forEach((line: any) => {
             const secName = line.sectionName || "PROJECT SPECIFICATIONS & WORKS";
             if (!groupedMap.has(secName)) {
                 groupedMap.set(secName, []);
             }
             groupedMap.get(secName)!.push({
                 id: line.id,
          ...
          const newSections: ProjectSectionForm[] = [];
          groupedMap.forEach((lines, sectionName) => {
              newSections.push({ sectionName, lines });
          });
  ...
  useEffect(() => {
    if (customers.length > 0 && clientId) {
       const customer: any = customers.find((c) => c.id === clientId);
        if (customer) {
  ...
  const handleCustomerChange = (id: string) => {
    setClientId(id);
    const customer: any = customers.find((c) => c.id === id);
    if (customer) {
      setClientEmail(customer.email || "");
      setClientPhone(customer.phoneNumber || "");
    }
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (result.success) {
        ...
        toast.error(result.errorMessage || "Failed to update project. Verify budget constraints.");
      }
    } catch (error: any) {
        toast.error(error?.response?.data?.errorMessage || "An unexpected error occurred. Verify budget constraints.");
    } finally {
      setLoading(false);
    }
  };
...
export default EditProjectWithContractForm;
