...
import { useCustomerStatement } from "../../hooks/useFinancialReports";
...
import { Loader2, Download, ArrowLeft, Users, Search } from "lucide-react";
...
const CustomerStatement: React.FC = () => {
  ...
  const { data: customersData } = useCustomers({ pageNumber: 1, pageSize: 1000 }, { isSupplier: false });
  const customers = customersData?.data?.data || [];
  
  const { data: response, isLoading } = useCustomerStatement(selectedCustomerId, fromDate, toDate);
    ...
    <div className="p-6 space-y-6">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-gray-500">{isRTL ? "كشف تفصيلي لمستحقات ومدفوعات العميل" : "Detailed statement of customer invoices and payments"}</p>
          </div>
        </div>
      ...
      <Card className="bg-gray-50 dark:bg-gray-800/50 border-none shadow-sm">
        <CardContent className="p-4 flex flex-wrap items-end gap-4">
          <div className="flex-1 min-w-[300px] flex flex-col gap-1">
            <Label className="text-xs">{isRTL ? "اختر العميل" : "Select Customer"}</Label>
            <Select value={selectedCustomerId} onValueChange={setSelectedCustomerId}>
              <SelectTrigger>
                <SelectValue placeholder={isRTL ? "اختر عميلاً..." : "Choose a customer..."} />
              </SelectTrigger>
              <SelectContent>
                {customers.map((c: any) => (
      ...
      {!selectedCustomerId ? (
        <div className="py-20 text-center flex flex-col items-center gap-4 border-2 border-dashed rounded-xl">
           ...
           <p className="text-gray-500">{isRTL ? "يرجى اختيار عميل لعرض كشف الحساب" : "Please select a customer to view their statement"}</p>
        </div>
      ) : (
        <>
