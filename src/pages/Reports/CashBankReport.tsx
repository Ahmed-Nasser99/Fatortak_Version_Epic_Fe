    ...
    <div className="p-6 space-y-6">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-2xl font-bold">{isRTL ? "حركة الصندوق والبنوك" : "Cash & Bank Movements"}</h1>
            <p className="text-sm text-gray-500">{isRTL ? "تقرير حركات المقبوضات والمدفوعات" : "Income and expense movement report"}</p>
          </div>
        </div>
      ...
      {isLoading ? (
        ...
        <div className="space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
             <SummaryCard title={isRTL ? "الرصيد الافتتاحي" : "Opening Balance"} amount={response.openingBalance} />
             <SummaryCard title={isRTL ? "إجمالي المقبوضات" : "Total Income"} amount={response.totalIncome ?? response.income ?? 0} variant="success" />
             <SummaryCard title={isRTL ? "إجمالي المدفوعات" : "Total Expense"} amount={response.totalExpense ?? response.expense ?? 0} variant="danger" />
             <SummaryCard title={isRTL ? "الرصيد الختامي" : "Closing Balance"} amount={response.closingBalance ?? response.balance ?? 0} isHighlight />
          </div>

          <MovementTable movements={response.movements} isRTL={isRTL} />
        </div>
      ) : (
              ...
              >
                ...
                <div className="flex gap-6 text-sm font-mono">
                  <div className="flex flex-col items-end">
                    ...
                    <span className="text-green-600">{formatCurrency(acc.totalIncome ?? acc.income ?? 0)}</span>
                  </div>
                  <div className="flex flex-col items-end">
                    <span className="text-[10px] text-gray-500 uppercase">{isRTL ? "المدفوعات" : "Expense"}</span>
                    <span className="text-red-600">{formatCurrency(acc.totalExpense ?? acc.expense ?? 0)}</span>
                  </div>
                  <div className="flex flex-col items-end min-w-[120px]">
                    <span className="text-[10px] text-gray-500 uppercase font-bold">{isRTL ? "الرصيد النهائي" : "Net Balance"}</span>
                    <span className="font-bold">{formatCurrency(acc.closingBalance ?? acc.balance ?? 0)}</span>
                  </div>
                </div>
              </div>
...
const MovementTable = ({ movements, isRTL, compact }: { movements: any[], isRTL: boolean, compact?: boolean }) => {
  return (
        ...
        <Table>
          <TableHeader>
            <TableRow className="hover:bg-transparent">
              ...
              <TableHead>{isRTL ? "البيان" : "Category"}</TableHead>
              ...
              <TableRow key={idx}>
                ...
                <TableCell className="text-sm">
                  <div className="flex flex-col">
                    ...
                    <span className="text-[10px] text-blue-600 dark:text-blue-400 font-mono">#{entry.entryNumber}</span>
                  </div>
                  {entry.projectName && <div className="text-[10px] text-gray-400 mt-1 italic">[{entry.projectName}]</div>}
                </TableCell>
                <TableCell className="text-right font-mono text-green-600">{(entry.income > 0 || entry.debit > 0) ? formatCurrency(entry.income || entry.debit) : '-'}</TableCell>
                <TableCell className="text-right font-mono text-red-600">{(entry.expense > 0 || entry.credit > 0) ? formatCurrency(entry.expense || entry.credit) : '-'}</TableCell>
              </TableRow>
            ))}
