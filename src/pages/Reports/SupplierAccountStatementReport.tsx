...
const SupplierAccountStatementReport: React.FC = () => {
  const { t, isRTL } = useLanguage();

  // State for filters
  const [selectedCustomerId, setSelectedCustomerId] = useState<string>("");
  const [accountType, setAccountType] = useState<"Sell" | "Buy">("Buy");
  ...
  const handleExport = async (format: "excel" | "pdf") => {
    if (!selectedCustomerId) {
      toast.error(t("selectSupplierFirst") || "Please select a supplier first");
      return;
    }
    ...
    try {
      ...
      a.href = url;
      a.download = `Supplier_Account_Statement_${new Date().toISOString().split('T')[0]}.${format === "excel" ? "xlsx" : "pdf"}`;
...
export default SupplierAccountStatementReport;
